...
Android 版本已经包含一体式底部导航、总览里的资产比例图、行动中心、总资产趋势、分布变化、单项资产趋势、本地快照、常用币种实时汇率、顶部设置入口、应用内更新页和下载入口、隐私模式、本地备份恢复，以及可折叠、可搜索和可筛选的资产管理区域。
...
`0.4.1` 之后 GitHub Actions 会复用稳定 debug 签名，后续下载通常可以直接覆盖安装。若从旧签名版本更新时仍提示“签名不一致”，需要先卸载旧版再安装一次新版。由于仓库是 private，手机浏览器需要登录有权限的 GitHub 账号才能下载。
